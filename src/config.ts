...
export async function readConfig(): Promise<Record<string, string>> {
  try {
    ...
    console.error(`\x1b[31mCould not read config file ${CONFIG_FILE}:\x1b[0m`, error);
    return {};
  }
}
...
export async function readProfile(): Promise<Record<string, string>> {
  try {
    ...
    console.warn(`\x1b[33mCould not read profile file ${PROFILE_FILE}:\x1b[0m`, error);
    return {};
  }
}
    ...
    try {
        ...
        for (const key in newData) {
            if (Object.prototype.hasOwnProperty.call(newData, key)) {
                ...
                if (newValue !== null) { // Only process non-null updates
                    ...
                    if (existingProfileData[key] !== newValueString) {
                         console.log(`  Updating profile key "${key}": "${existingProfileData[key]}" -> "${newValueString}"`);
        ...
        if (profileChanged) {
             console.log('Profile data changed, writing updates back to file...');
             const updatedProfileLines: string[] = [];
             for (const key in existingProfileData) {
                 if (Object.prototype.hasOwnProperty.call(existingProfileData, key)) {
                      updatedProfileLines.push(`${key}: ${existingProfileData[key]}`);
                 }
             }
             ...
             console.log('No changes detected in profile data. File not updated.');
        }

    } catch (error) {
        console.error("\x1b[31mError during direct profile update or file write:\x1b[0m", error);
    }
} 