...
async function main() {
    ...
    try {
        ...
        if (!loggedIn) {
            console.warn(
                `Timed out waiting for login redirection to ${homeUrl} after ${
                    loginCheckTimeout / 1000
                } seconds. Current URL: ${page.url()}. Proceeding anyway...`
            );
        ...
        if (!eventPageUrl)
            throw new Error(`EVENT_CALENDAR_URL not found in config.txt`);
        ...
        console.log(
            `Waiting for event card links using selector: ${eventCardLinkSelector}`
        );
        ...
        for (const link of allEventLinks) {
            ...
            const registrationSuccess = await processEventPage(
                page,
                context,
                link,
                config
            ...
            if (registrationSuccess) {
                ...
                console.warn(
                    `Processing failed for ${link}. Adding to failures list.`
                );
        ...
        console.error(
            "\x1b[31mAn error occurred in main execution:\x1b[0m",
            error
        );
    } finally {
